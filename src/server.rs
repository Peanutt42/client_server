...
use crate::{ClientId, ClientPacket, RawMessageData};
...
impl Server {
    ...
    pub fn broadcast(&mut self, message: &RawMessageData, ignored_client: ClientId) -> Result<()> {
        self.shared_data.lock().unwrap().broadcast(
            &InternalServerPacket::ServerToClient(message.clone()),
            ignored_client,
        )
    }

    pub fn send(&mut self, message: &RawMessageData, client_id: ClientId) -> Result<()> {
        self.shared_data.lock().unwrap().send(
            &InternalServerPacket::ServerToClient(message.clone()),
            client_id,
        )
    }
    ...
    pub fn update_ping(&mut self) -> Result<()> {
        ...
        for (client_id, client) in shared_data.clients.iter_mut() {
            let now = Instant::now();
            if (client.ping_send_time.is_some()
                && now.duration_since(client.ping_send_time.unwrap()) >= ping_interval)
                || client.ping_send_time.is_none()
            {
                if let Err(e) = SharedData::stream_send(
    ...
    pub fn get_client_ip_address(&self, client_id: ClientId) -> Option<String> {
        if let Ok(shared_data) = self.shared_data.lock() {
            shared_data
                .clients
                .get(&client_id)
                .map(|client| client.stream.peer_addr().unwrap().to_string())
        } else {
            None
        }
    }
    ...
    pub fn kick_client(&mut self, client_id: ClientId) {
        if let Ok(mut shared_data) = self.shared_data.lock() {
            ...
            shared_data
                .broadcast(&InternalServerPacket::ClientKicked(client_id), client_id)
                .expect(
                    "failed to send notification to all other clients that one client got kicked",
                );
