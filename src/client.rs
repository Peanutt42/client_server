...
impl Client {
        ...
        A: std::net::ToSocketAddrs,
    {
        ...
        Ok(Self::new(
            NetworkingStream::Tcp(listen_stream),
            NetworkingStream::Tcp(send_stream),
        ))
    }
    ...
    pub fn send_to(&mut self, client_to_send_to: ClientId, message: &RawMessageData) -> Result<()> {
        self.shared_data
            .lock()
            .unwrap()
            .send_packet(&InternalClientPacket::PersonalMessage(
                client_to_send_to,
                message.clone(),
            ))
    }
    pub fn send_to_all(&mut self, message: &RawMessageData) -> Result<()> {
        self.shared_data
            .lock()
            .unwrap()
            .send_packet(&InternalClientPacket::BroadcastMessage(message.clone()))
    }
    pub fn send_to_server(&mut self, message: &RawMessageData) -> Result<()> {
        self.shared_data
            .lock()
            .unwrap()
            .send_packet(&InternalClientPacket::ServerMessage(message.clone()))
    }
