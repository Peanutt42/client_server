use client_server::{ClientMessage, Server};
use std::{
...
fn main() {
    let mut server =
        Server::bind_tcp("0.0.0.0:8080").expect("failed to bind server to 0.0.0.0:8080");
    ...
    loop {
        {
            ...
            while let Some(input) = input_commands.pop_back() {
                if input == "/exit" {
                    ...
                    server
                        .broadcast_all(&Vec::from(input.as_bytes()))
        ...
        while let Some(packet) = server.get_packet() {
            let client_address = server
                .get_client_ip_address(packet.author)
